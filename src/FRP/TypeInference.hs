...
-- Bottom unifies with anything, but should never hit this case
unifies (TyPrim _ TyBot) t = return emptyUnifier
unifies t (TyPrim _ TyBot) = return emptyUnifier
...
-- Does not actually only return the type, but rather the collection of all
