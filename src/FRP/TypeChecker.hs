...
checkTerm ctx term = case term of
  ...
  TmLit a l              -> case l of
    ...
    LUndefined -> return (TyPrim a TyBot, QNow)
  TmBinOp a op l r       -> do
  ...
  TmOut a  _ty trm    -> error "type-checking for TmOut not implemented"
  TmInto a _ty trm    -> error "type-checking for TmInto not implemented"
  where
